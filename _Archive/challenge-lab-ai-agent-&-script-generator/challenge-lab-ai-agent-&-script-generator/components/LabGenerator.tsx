// LabGenerator.tsx - Updated to support multiple AI providers
import React, { useState, useCallback, ChangeEvent, useEffect, useRef } from 'react';
import { AGENTS, AGENT_1_SERIES_COMBINED_PROMPT, AGENT_1_SINGLE_PROMPT } from '../constants';
...
export const LabGenerator: React.FC<LabGeneratorProps> = ({ navigateToScorer }) => {
  ...
  const handleAgentSelect = useCallback(async (selectedAgent: Agent) => {
    ...
    if (lastDoneAgent && selectedAgent.id > lastDoneAgent.id) {
      ...
      agentsToRun = activeAgents.filter(a => a.id > lastDoneAgentId && a.id <= selectedAgent.id);
    } else {
      setProcessedContent('');
      setLabSeries([]);
      setProcessedLabs([]);
      ...
      agentsToRun = activeAgents.filter(a => a.id <= selectedAgent.id);
    }
    ...
    // Create AI service instance with current configuration
    ...
    try {
      for (const agent of agentsToRun) {
        ...
        if (agent.id === 1) {
            const promptToUse = labCreationMode === 'single' ? AGENT_1_SINGLE_PROMPT : AGENT_1_SERIES_COMBINED_PROMPT;
            setProcessedLabs([]);
            
            const result = agent.id === selectedAgent.id
                ? await aiService.processTextStream(promptToUse, initialContent, {
                    onChunk: (chunk) => setProcessedContent(prev => prev + chunk),
                    onStatusUpdate: setStatusMessage,
                    isCancelledRef
                  })
                : await aiService.processText(promptToUse, initialContent, {
                    onStatusUpdate: setStatusMessage,
                    isCancelledRef
                  });
            ...
            for (let i = 0; i < labsToProcess.length; i++) {
                ...
                const result = await aiService.processText(agent.prompt, lab.content, {
                  onStatusUpdate: setStatusMessage,
                  isCancelledRef
                });
            ...
            const result = (agent.id === selectedAgent.id)
              ? await aiService.processTextStream(agent.prompt, currentContent, {
                  onChunk: (chunk) => setProcessedContent(prev => prev + chunk),
                  onStatusUpdate: setStatusMessage,
                  isCancelledRef
                })
              : await aiService.processText(agent.prompt, currentContent, {
                  onStatusUpdate: setStatusMessage,
                  isCancelledRef
                });
  ...
  }, [initialContent, workflowMode, labCreationMode, agentStatuses, history, labSeries, processedLabs, fileName, activeAgents, aiConfig]);
