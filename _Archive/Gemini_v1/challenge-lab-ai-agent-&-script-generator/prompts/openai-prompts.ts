 ...
 *
 * WORKING VERSION - These prompts work perfectly with OpenAI
 */
...
The user will provide the following configuration values in their input:
- **Number of Labs:** {{NUM_LABS}} (This value will be dynamically inserted by the application)
- **Requirements per Lab:** {{NUM_REQUIREMENTS}} (This value will be dynamically inserted by the application)

**CRITICAL STRUCTURE REQUIREMENTS:**
1.  **Total Labs:** The series MUST contain exactly **{{NUM_LABS}} distinct labs**. Read this value from the user's input.
2.  **Lab Separation:** Each lab MUST be separated by a line containing only \`---\`.
3.  **Requirements per Lab:** Each lab MUST contain exactly **{{NUM_REQUIREMENTS}} requirements**. Read this value from the user's input.
...
2.  **Move the bulleted list of steps into a Guided Hint block:**
    ...
    - **CRITICAL:** Remove the original bulleted steps from their position under the task line - they should ONLY appear inside the hint block, not duplicated outside it.
